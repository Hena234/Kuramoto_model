...
# ╔═╡ f3f3b310-96e0-11ef-260e-d935dcdaab1f
begin
...
# ╔═╡ 3a95b041-3ef2-435b-bac2-6c75c5bcfe55
md"""
# Term Paper 2
 """

# ╔═╡ fe644b77-dc78-4806-bd83-60a104d604ca
md"""
### Kuramoto Model for K ranging from 0.1 to 10
	"""


# ╔═╡ f485f794-2a22-405a-b908-660c5578fc2e
begin
	...
	ω = ω1.-mean(ω1)
	tspan = (0.0, 10.0)
	u0 = 2π .*rand(N)            # Initial phases
    Ks = range(0.1, stop=10.0, length=20)  # Values of K for animation

    # Initialize animation
    anim = @animate for K in Ks
        # Define the Kuramoto model ODE with dynamic K
        function Kuramoto!(du, u, p, t)
            for i in 1:N
                du[i] = ω[i] + K / N * sum(sin(u[j] - u[i]) for j in 1:N)
            end
        end

        # Set up and solve the ODE problem for the current K
        prob_k = ODEProblem(Kuramoto!, u0, tspan)
        sol_k = solve(prob_k, saveat=0.1)

        # Plot the phase of each oscillator over time
        plot(sol_k.t, hcat(sol_k.u...)', xlabel="Time", ylabel="Phase",
             title="Kuramoto Model for K = $K", legend=false, lw=0.3)
    end

    # Save and display the animation
    gif(anim, "Kuramoto_Animation.gif", fps=5)
end


# ╔═╡ cd300ce0-9205-436c-aae6-0d1aa1975a49
md"""
### Order Parameter vs Time Graph for different values of K
"""


# ╔═╡ ebb6f8f9-1b0f-4b8b-964f-e0aad99714ad
begin
    # Initialize a plot
    p = plot()
    
    # Iterate over different coupling strengths K from 0 to 10
    for K in 0:10
        # Define the Kuramoto model differential equation
        function Kuramoto!(du, u, p, t)
            for i in 1:N
                du[i] = ω[i] + K / N * (sum(sin(u[j] - u[i]) for j in 1:N))
            end
        ...
        # Set up and solve the ODE problem for the current K
        prob = ODEProblem(Kuramoto!, u0, tspan)
        global sol = solve(prob, saveat=0.1)

        # Initialize the order parameter array
        r = zeros(length(sol.t))
        
        # Function to compute the synchronization order parameter over time
        function sync!(r, sol)
            for i in 1:length(sol.t)
                r[i] = abs(((1 / N) * sum(exp(im * sol.u[i][j]) for j in 1:N)) / exp(im * sum(sol.u[i][j] for j in 1:N) / N))
            end
            return r
        end

        # Compute the order parameter for the current solution
        r = sync!(r, sol)
        r∞[K + 1] = r[end]

        # Plot the order parameter vs. time for the current K
        plot!(sol.t, r, xlabel="Time", ylabel="Order Parameter", title="Order Parameter")
    end
    
    # Finalize the plot without a legend
    plot!(legend=false)
end

# ╔═╡ b6ac36dc-f56a-4dca-8979-d727c1838bb8
md"""
### Order Parameter vs Coupling Strength Grpah
"""

# ╔═╡ d09e28b5-ca1e-4229-b537-8a0fc97a0f8f
#plot the order parameter vs coupling strength graph
plot(0:10,r∞,xlabel= "Coupling Strength" , ylabel= "Order Parameter" ,title="Order Parameter vs Coupling Strength")


# ╔═╡ fd9bc2d8-f918-43f0-9897-7a732f66e17e
md"""
### Firefly Synchronization based on Kuramoto Model
"""

# ╔═╡ 44bce47b-c388-44a1-903c-dfe3e9738921
begin
	
	
	# Basic parameters
	N_f = 20                     # Number of fireflies
	ω_f = randn(N_f) * 0.1         # Natural frequencies
	K_f = 2.0                    # Coupling strength
	tspan_f = (0.0, 50.0)        # Simulation timespan
	u0_f = rand(N_f) * 2π          # Initial phases
	
	# Random positions for fireflies
	positions = [(rand() * 10, rand() * 10) for _ in 1:N_f]
	...
	function kuramoto_f!(du, u, p, t)
	    for i in 1:N_f
	        du[i] = ω_f[i]
	        for j in 1:N_f
	            du[i] += (K_f / N_f) * sin(u[j] - u[i])
	        end
	    end
	end
	
	# Solve the system
	prob_f = ODEProblem(kuramoto_f!, u0_f, tspan_f)
	sol_f = solve(prob_f, saveat=0.2)  # Adjusted saveat for fewer frames
	
	# Create animation
	anim_f = @animate for t in 1:length(sol_f.t)
	    # Plot the canvas
	    plot(xlims=(0, 10), ylims=(0, 10),
	         background=:black,
	         legend=false,
	         title="Firefly Synchronization (t=$(round(sol_f.t[t], digits=1)))")
	    
	    # Plot each firefly
	    for i in 1:N_f
	        # Map phase to brightness (0 to 1)
	        brightness = (1 + sin(sol_f.u[t][i])) / 2
	        
	        # Plot firefly as a glowing circle
	        scatter!([positions[i][1]], [positions[i][2]],marker=:circle,markersize=12,color=RGB(brightness, brightness, 0),  
			alpha=brightness)
	        ...
	        scatter!([positions[i][1]], [positions[i][2]],
	                marker=:circle,
	                markersize=20,
	                color=RGB(brightness/2, brightness/2, 0),
	                alpha=brightness/2)
	    end
	end
	
	# Save animation
	gif(anim_f, "fireflies_2d.gif", fps=20)
...
manifest_format = "2.0"
project_hash = "c1ec71c497dc3b09f206353bbdbe1024a14416d9"
